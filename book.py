...
class Book:
    ...
    def __init__(self):
        ...
        self.series = None
        self.series_order = 0
        self.previous_in_series = 0
        self.next_in_series = 0
        self.publishing_order = 0
        self.previous_publishing_order = 0
        self.next_publishing_order = 0
        self.chronological_order = 0
        self.previous_chronological_order = 0
        self.next_chronological_order = 0
        self.births = []
    ...
    def is_born(self, cat):
        ...
        if isinstance(clan, Clan):
            ...
            clan.current_kittens.append(cat.current_name)
            if clan == Wind_Clan:
    ...
    def becomes_apprentice(self, cat1, cat2):
        ...
        if isinstance(clan, Clan):
            if cat1.current_name in clan.current_kittens:
                clan.current_kittens.remove(cat1.current_name)
            if cat1.current_name in clan.current_medicine_apprentices:
                clan.current_medicine_apprentices.remove(cat1.current_name)
        cat1.other_names.append(cat1.current_name)
        cat1.current_name = cat1.prefix + "paw"
        self.new_apprentices.append(cat1.current_name)
        cat1.mentors.append(cat2)
        cat1.previous_ranks.append(cat1.rank)
        cat1.rank = "Apprentice"
        cat1.became_apprentice.append(self.title)
        if isinstance(clan, Clan):
            clan.current_apprentices.append(cat1.current_name)
    ...
    def becomes_med_apprentice(self, cat1, cat2):
        ...
        if isinstance(clan, Clan):
            if cat1.current_name in clan.current_kittens:
                clan.current_kittens.remove(cat1.current_name)
            if cat1.current_name in clan.current_apprentices:
                clan.current_apprentices.remove(cat1.current_name)
        cat1.other_names.append(cat1.current_name)
        cat1.current_name = cat1.prefix + "paw"
        self.new_medicine_apprentices.append(cat1.current_name)
        cat1.mentors.append(cat2)
        cat1.previous_ranks.append(cat1.rank)
        cat1.rank = "Medicine Apprentice"
        cat1.became_apprentice_medicine.append(self.title)
        if isinstance(clan, Clan):
            clan.current_medicine_apprentices.append(cat1.current_name)

    def becomes_mentor(self, cat1, cat2):
        """Function to become a mentor. The arguments are cat1 (the mentor) and cat2 (the apprentice)."""
        self.new_mentors.append(cat1.current_name)
    ...
    def becomes_warrior(self, cat):
        ...
        if isinstance(clan, Clan):
            if cat.current_name in clan.current_apprentices:
                clan.current_apprentices.remove(cat.current_name)
            if cat.current_name in clan.current_medicine_cats:
                clan.current_medicine_cats.remove(cat.current_name)
        cat.other_names.append(cat.current_name)
        cat.current_name = cat.prefix + cat.suffix
        self.new_warriors.append(cat.current_name)
        cat.previous_ranks.append(cat.rank)
        cat.rank = "Warrior"
        cat.became_warrior.append(self.title)
        if isinstance(clan, Clan):
            clan.current_warriors.append(cat.current_name)

    def becomes_medicine_cat(self, cat):
        """When a cat becomes the clan's medicine cat."""
        clan = cat.affiliation
        if isinstance(clan, Clan):
            if cat.current_name in clan.current_medicine_apprentices:
                clan.current_medicine_apprentices.remove(cat.current_name)
            if cat.current_name in clan.current_warriors:
                clan.current_warriors.remove(cat.current_name)
        cat.other_names.append(cat.current_name)
        cat.current_name = cat.prefix + cat.suffix
        self.new_medicine_cats.append(cat.current_name)
        cat.previous_ranks.append(cat.rank)
        cat.rank = "Medicine Cat"
        cat.became_medicine_cat.append(self.title)
        if isinstance(cat.affiliation, Clan):
            clan.current_medicine_cats.append(cat.current_name)
    ...
    def becomes_queen(self, cat):
        ...
        if isinstance(clan, Clan):
            if cat.current_name in clan.current_warriors:
                clan.current_warriors.remove(cat.current_name)
            clan.current_queens.append(cat.current_name)
        cat.previous_ranks.append(cat.rank)
        cat.rank = "Queen"
        self.new_queens.append(cat.current_name)
    ...
    def becomes_elder(self, cat):
        ...
        if isinstance(clan, Clan):
            ...
            if cat.current_name in clan.current_medicine_cats:
                clan.current_medicine_cats.remove(cat.current_name)
            if cat.current_name in clan.current_queens:
                clan.current_queens.remove(cat.current_name)
            clan.current_elders.append(cat.current_name)
        cat.previous_ranks.append(cat.rank)
        cat.rank = "Elder"
        self.new_elders.append(cat.current_name)
    ...
    def becomes_deputy(self, cat):
        ...
        if isinstance(clan, Clan):
            ...
            if cat.current_name in clan.current_queens:
                clan.current_queens.remove(cat.current_name)
            clan.current_deputy = cat.current_name
        cat.previous_ranks.append(cat.rank)
        cat.rank = "Deputy"
        self.new_deputies.append(cat.current_name)
    ...
    def becomes_leader(self, cat):
        ...
        clan = cat.affiliation
        if isinstance(cat.affiliation, Clan):
            if cat.current_name in clan.current_deputy:
                clan.current_deputy.remove(cat.current_name)
            if cat.current_name in clan.current_queens:
                clan.current_queens.remove(cat.current_name)
            cat.other_names.append(cat.current_name)
        ...
        cat.previous_ranks.append(cat.rank)
        cat.rank = "Leader"
        clan.current_leader = cat.current_name
        self.new_leaders.append(cat.current_name)
    ...
    def dies(self, cat):
        ...
        if cat.lifecount == 0:
            ...
            if isinstance(cat.affiliation, Clan):
                ...
                if cat.current_name in clan.current_kittens:
                    clan.current_kittens.remove(cat.current_name)
                if cat.current_name in clan.current_apprentices:
                    clan.current_apprentices.remove(cat.current_name)
                if cat.current_name in clan.current_medicine_apprentices:
                    clan.current_apprentices.remove(cat.current_name)
                ...
                if cat.current_name in clan.current_queens:
                    clan.current_queens.remove(cat.current_name)
                if cat.current_name in clan.current_elders:
                    clan.current_elders.remove(cat.current_name)
                if cat.current_name in clan.current_deputy:
                    clan.current_deputy.remove(cat.current_name)
                if cat.current_name in clan.current_leader:
                    clan.current_leader.remove(cat.current_name)
    ...
    def double_dies(self, cat):
        ...
        clan = cat.affiliation
        if isinstance(clan, Clan):
            clan.current_members.remove(cat.current_name)
            clan.previous_members.append(cat.current_name)
            clan = None
        cat.double_death = self.title
        self.double_deaths.append(cat.current_name)
    ...
    def leaves_clan(self, cat):
        ...
        self.left_clan.append(cat.current_name)

    def joins_clan(self, cat, clan, rank):
        """When a cat joins a clan. Takes the cat and the rank they join into as arguments."""
        cat.previous_affiliations.append(cat.affiliation)
        clan.current_members = clan.current_members + cat.current_name
        cat.joined_clan.append(self.title)
        self.joined_clan.append(cat.current_name)
        if rank == "Kitten":
            clan.current_kittens.append(cat.current_name)
        if rank == "Apprentice":
            clan.current_apprentices = clan.current_apprentices + cat.current_name
        if rank == "Medicine Apprentice":
            clan.current_medicine_apprentices = clan.current_medicine_apprentices + cat.current_name
        if rank == "Warrior":
            clan.current_warriors = clan.current_warriors + cat.current_name
        if rank == "Medicine Cat":
            clan.current_medicine_cats = clan.current_medicine_cats + cat.current_name
        if rank == "Queen":
            clan.current_queens = clan.current_queens + cat.current_name
        if rank == "Elder":
            clan.current_elders = clan.current_elders + cat.current_name
        if rank == "Deputy":
            clan.current_deputy = cat.current_name
        if rank == "Leader":
            clan.current_leader = cat.current_name
